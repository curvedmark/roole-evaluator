...
Evaluator.prototype.evalJs = function (content) {
	var module = { exports: {} };
	content = '!function(module,exports){' + content + '}(module,module.exports)';
	eval(content);
...
Evaluator.prototype.visit_import = function (importNode) {
	return this.visit(importNode.children).then(function (children) {
		...
		var promise = isAbsolute(filename) || prefixedPathRe.test(filename)
			? this.importModule(filename)
			: this.importLib(filename);
...
Evaluator.prototype.importModule = function (filename) {
	if (filename.slice(-1) === '/') return this.importDir(filename);

	return anyFirst([
		this.importFile(filename),
		this.importDir(filename)
	]);
};

Evaluator.prototype.importDir = function (dirname) {
	var pkg = this.resolvePath(path.join(dirname, 'package.json'));
	pkg = this.load(pkg).then(function (content) {
		var main = JSON.parse(content).main;
		main = path.join(dirname, main);
		return this.importFile(main);
	});

	var idx = path.join(dirname, 'index.roo')
	idx = this.importFile(idx);
	var promises = [pkg, idx];

	return anyFirst(promises);
};

Evaluator.prototype.importFile = function (filename) {
	filename = this.resolvePath(filename);

	// only import once
	if (this.imported[filename]) return this.visit(null);

	return this.load(filename).then(function (content) {
		this.imported[filename] = true;

		if (path.extname(filename) === '.js') {
			var exports = this.evalJs(content);
		...
		var origFilename = this.filename;
		this.filename = filename;
		return this.eval(content, { filename: filename }).then(function (stylesheet) {
...
Evaluator.prototype.importLib = function (dirname) {
	var promises = [];
	...
	for (var i = parts.length - 1; i >= 0; --i) {
		...
		var promise = this.importModule(lib);
...
Evaluator.prototype.load = function (filename) {
	...
	if(typeof content === 'string') return promise.fulfill(content, this);
...
Evaluator.prototype.visit_module = function (mod) {
	...
	return this.visit(mod.children[0]).then(function (name) {
		...
		this.moduleName = modName + nameVal + sepVal;

...
function Evaluator(options) {
	...
	this.scope = new Scope([builtin, new Global(options), {}]);
}
...
Evaluator.prototype.visit_import = function (importNode) {
	return this.visit(importNode.children).then(function (children) {
		...
		var req = url.children[0];
		if (protocolRe.test(req) || req.slice(-4) === '.css') return;

		// import file
		var dirname = importNode.loc.filename;
		if (dirname.slice(-1) !== path.sep) dirname = path.dirname(dirname);

		var promise = moduleRe.test(req)
			? this.importModule(req, dirname)
			: this.importLib(req, dirname);

		return promise.then(function (file) {
			// file already imported
			if (file === null) return null;

			// swap out the global scope so variables
			// like __dirname have a new value
			var opts = {
				filename: file.name,
				out: this.options.out
			};
			var global = this.scope.frames[1];
			this.scope.frames[1] = new Global(opts);

			// eval the imported file
			return this.eval(file.content, opts).then(function (stylesheet) {
				this.scope.frames[1] = global;
				return stylesheet.children;
			});
		}, function (err) {
			if (err.errno === 34) throw new RooleError("Cannot find module '" + req + "'", importNode);
			throw err;
		})
	});
};

Evaluator.prototype.importModule = function (req, dirname) {
	if (req.slice(-1) === path.sep) return this.importDir(req, dirname);

	var extReq = path.extname(req) ? req : req + '.roo';
	return anyFirstPromise([
		this.importFile(extReq, dirname),
		this.importDir(req, dirname)
	]);
};

Evaluator.prototype.importDir = function (req, dirname) {
	dirname = path.resolve(dirname, req);
...
Evaluator.prototype.importFile = function (req, dirname) {
	var filename = path.resolve(dirname, req);
...
Evaluator.prototype.importLib = function (req, dirname) {
	var promises = [];
	...
	for (var i = parts.length - 1; i >= 0; --i) {
		...
		var promise = this.importModule(req, dirname);

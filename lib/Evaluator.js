...
function Evaluator(options) {
	...
	this.imported = {};
	this.scope = new Scope(options.scope || [builtin, {}]);
}
...
Evaluator.prototype.visit_assignment = function (assign) {
	...
	return this.visit(member.children[0]).then(function (l) {
		...
		var vals = val.type === 'list' ? val.children.slice(0) : [val];
		if (range.reversed) vals.reverse();

		var isAssignOp = op !== '?=' && op !== '=';

		if (range.to <= 0) {
			// do nothing if `$list = []; $list[-2..-1] += 1;`
			if (isAssignOp) return null;

			var sep;
			if (items.length > 1) {
				sep = items[items.length - 2];
			} else if (vals.length > 1) {
				sep = vals[vals.length - 2];
			} else {
				sep = { type: 'separator', children: [' '], loc: val.loc };
			}
			...
			items = vals.concat(extras, items);
		} else if (range.from >= len) {
			// do nothing if `$list = []; $list[1..2] += 1;`
			if (isAssignOp) return null;
			...
			items = items.concat(extras, vals);
		} else {
			var from = Math.max(range.from, 0);
			var to = Math.min(range.to, len);

			if (from === to) {
				// do nothing if `$list = [0]; $list[0...0] += 1;`
				if (isAssignOp) return null;
				...
				if (len === 1) {
					sep = vals.length > 1
						? vals[vals.length - 2]
						: { type: 'separator', children: [' '], loc: val.loc };
				} else {
					sep = items[items.length -2];
				}
				from = from * 2;
				items = items.slice(0, from).concat(vals, sep, items.slice(from));
			} else if (to - from === 1 && isAssignOp) {
				op = op.charAt(0);
				...
				from = from * 2
				to = to * 2 - 1;
				items = items.slice(0, from).concat(vals, items.slice(to))
			}
		}

		// replace list.children with items
		list.children.length = 0;
		list.children.push.apply(list.children, items);
...
Evaluator.prototype.visit_variable = function (variable) {
	...
	if (val.type === 'biv') return val.children[0](variable, this.options);
...
Evaluator.prototype.visit_import = function (importNode) {
	return this.visit(importNode.children).then(function (children) {
		...
		// ignore url starting with protocol or ending with .css
		var path = url.children[0];
		if (protocolRe.test(path) || path.slice(-4) === '.css') return;
		...
		var dirname;
		if (result) {
			this.urlHead = result[1];
			this.urlTail = result[3];
			filename = result[2] || '/';
		} else {
			this.urlHead = '';
			this.urlTail = '';
...
Evaluator.prototype.importModule = function (path, dirname) {
	if (path.slice(-1) === Path.sep) return this.importDir(path, dirname);

	var extPath = Path.extname(path) ? path : path + '.roo';
	return anyFirstPromise([
		this.importFile(extPath, dirname),
...
Evaluator.prototype.importFile = function (path, dirname) {
	var filename = this.urlHead + Path.resolve(dirname, path) + this.urlTail;
...
Evaluator.prototype.visit_call = function (call) {
	return this.visit(call.children[0]).then(function (func) {
		...
		return this.visit(argList).then(function (argList) {
			// builtin function
			if (func.type === 'bif') return func.children[0](call, this.options);
